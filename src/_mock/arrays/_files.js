...
const FOLDER_NAME = [
  { folder: 'CIBIL', image: 'https://commerceclub.in/wp-content/uploads/2014/12/cibil-logo.jpg' },
  {
    folder: 'Equifax',
    image:
      'https://res.cloudinary.com/crunchbase-production/image/upload/c_lpad,h_256,w_256,f_auto,q_auto:eco,dpr_1/mzr023wwoekxlpt8ctpl',
  },
  {
    folder: 'CRIF',
    image:
      'https://media.glassdoor.com/sqll/506323/crif-lending-solutions-squarelogo-1427099760093.png',
  },
  {
    folder: 'Experian',
    image: 'https://logos-world.net/wp-content/uploads/2021/03/Experian-Logo.png',
  },
];
const DB_FILE_FOLDER_NAME = ['MySQL', 'PostgreSQL', 'Oracle', 'SnowSQL', 'Downlaod CSV File'];
...
export const _folders = FOLDER_NAME.map(({ folder, image }, index) => ({
  ...
  url: FILE_URL[index],
  image,
  tags: ['CIBIL', 'Equifax', 'CRIF', 'Experian'],
  dateCreated: _mock.time(index),
  dateModified: _mock.time(index),
}));

export const _dbFilefolders = DB_FILE_FOLDER_NAME.map(( folder, index) => ({
  id: `${_mock.id(index)}_folders`,
  name: folder,
  size: GB / ((index + 1) * 10),
  type: 'folder',
  ...
  shared: SHARED_PERSON.slice(index, 5),
  url: FILE_URL[index],
  tags: ['MySQL', 'PostgreSQL', 'Oracle', 'SnowSQL', 'Downlaod CSV File'],
  dateCreated: _mock.time(index),
  dateModified: _mock.time(index),
}));
...
export const _dbFileFiles = [..._dbFilefolders];
