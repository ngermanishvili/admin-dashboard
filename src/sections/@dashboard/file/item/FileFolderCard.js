...
export default function FileFolderCard({
  ...
  return (
    <Link style={{textDecoration:"none"}} state={{subCategory,category}} to={routeTo}>
      <Card
...
{/* {noLogo ? null : ((showCheckbox || selected) && onSelect ? (
      ...
      <FileNewFolderDialog
        ...
        onUpdate={() => {
          ...
          console.log('UPDATE FOLDER', subCategoryName);
        }}
        folderName={subCategoryName}
        onChangeFolderName={(event) => setsubCategoryName(event.target.value)}
      />
